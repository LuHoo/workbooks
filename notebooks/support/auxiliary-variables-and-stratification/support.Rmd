...
```{r}
mySample <- FSaudit::select(mySample)
...
```{r}
arSample <- FSaudit::select(arSample, seed = 1)
```
<!-- ADA:BEGIN lang=python mode=override kind=code -->
```{r}
ada_set_context("3.7")
...
```{r}
cumul <- FSaudit::select(cumul, seed = 12345)
head(cumul$sample)
```
<!-- ADA:BEGIN lang=python mode=override kind=code -->
```{r}
...
error_distributions <- gridExtra::grid.arrange(p1, p2, p3, p4, ncol=2)
...
```{r}
myStrat <- FSaudit::select(myStrat, seed = 12345)
head(myStrat$sample)
```
