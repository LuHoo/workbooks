...
run_full_export_set_equivalence_test <- function() {
  ...
  writeLines(c(
    ...
    "  message('CONFIG_START: ', config$id)",
    "  if (identical(engine, 'default')) {",
    "    result <- tryCatch({",
    "      suppressMessages(export_workshop_by_config(config = config, output_dir = out_dir, enable_traceability = FALSE))",
    "      NULL",
    "    }, error = function(e) e)",
    "  } else {",
    "    result <- tryCatch({",
    "      suppressMessages(export_workshop_by_config(config = config, output_dir = out_dir, parser_engine = engine, enable_traceability = FALSE))",
    "      NULL",
    "    }, error = function(e) e)",
    "  }",
    "  if (!is.null(result)) {",
    "    stop(sprintf('CONFIG_FAILED:%s:%s', config$id, conditionMessage(result)))",
    "  }",
    "}"
  ), runner_script)
