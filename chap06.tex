...
As we are about to enter the arena of \textbf{substantive analytical procedures}\index{substantive analytical procedures} (SAP)\index{SAP}, it is wise to first review the professional literature on these procedures. 
...
\bigskip
Now what are SAPs? The definition in the Standards\footnote{ISA 520.4, AU-C 520.4} emphasizes three important concepts. First, it is the \textbf{evaluation of financial information}, which suggests that analytical procedures will be used to understand or test financial statement relationships or balances.

Secondly, the evaluation takes place through the analysis of \textbf{plausible relationships}. 
...
Third and final, the standards suggest that \textbf{both financial and non-financial data} can be used to form the relationships of financial information, and therefore, also in forming expectations. 
...
\textsc{remember}
\begin{compactitem}
\item define a substantive analytical procedure (SAP) as outlined in auditing standards.
...
\textsc{understand}
\begin{compactitem}
\item explain how effectiveness risk and efficiency risk influence the determination of the acceptable difference in SAPs.
...
\textsc{apply}
\begin{compactitem}
\item use reliability and disaggregation principles to obtain relevant data for SAP.
...
\textsc{analyze}
\begin{compactitem}
\item assess the role of effectiveness risk and efficiency risk in SAP outcomes.
\item break down the components of predictive modeling to understand their influence on the precision of the expectation.
\end{compactitem}

\textsc{evaluate}
\begin{compactitem}
\item judge whether the objectives of the SAP have been achieved based on the analysis of deviations.
\item recommend improvements to the SAP process for enhancing precision and achieving audit goals.
\end{compactitem}

\textsc{create}
\begin{compactitem}
\item develop a comprehensive SAP plan, incorporating the phases of planning, data collection, modeling, and evaluation.
...
\section*{Objective}
Apply the requirements and guidance from professional standards when performing SAPs in response to the relevant \textbf{risk of material misstatement}\index{risk of material misstatement} (\textbf{RMM}\index{RMM}) for the current year's sales. 
...
\item Plan the SAP.
\begin{compactitem}
\item Determine the financial statement item or account and related assertions for which the SAP is to provide audit evidence and the specific objectives of the SAP. (Section \ref{sub:account_and_assertions})
\item Identify the assessed risks of material misstatement to which the SAP is intended to respond (desired level of assurance). (Section \ref{sub:assessed_risk_of_material_misstatement})
\item Determine whether a SAP is suitable to address the RMM. (Section \ref{sub:suitability_of_the_sap})
\item Determine the plausibility of the relationship. (Section \ref{sub:plausibility_of_the_relationship})
\item Determine the desired precision of the expectation. (Section \ref{sub:desired_precision})
\item Determine the SAP (for example, trend analysis, ratio analysis, non-statistical predictive modelling, regression analysis) that is most likely to meet the objectives. (Section \ref{sub:choice_of_procedure})
\end{compactitem}
\item Obtain the data from which the expectation of recorded amounts or ratios is to be developed, including
\begin{compactitem}
\item the relevance of the data; (Section \ref{sub:relevance_of_the_data})
\item the reliability of the data; (Section \ref{sub:reliability})
\item sources of the data for those variables; (Section \ref{sub:source_of_the_data}) and
\item levels of disaggregation of the data. (Section \ref{sub:disaggregation})
\end{compactitem}
\item Perform the SAP.
\begin{compactitem}
\item Develop the model to be used, when applicable. (Volume 1, Section \ref{sec:modeling} to \ref{sec:testing_model_assumptions})
\item Evaluate whether the expectation is sufficiently precise and, if not, the actions to take to increase the precision. (Section \ref{sub:evaluation_of_precision})
\item Determine the amount of difference from the expectation that can be accepted without further investigation. (Section \ref{sub:acceptable_difference})
\item Develop the expectation of the recorded amount or ratio. (Section \ref{sub:expectation_development})
\item Compare the expectation to the recorded amount or ratio.
\end{compactitem}
\item Evaluate and respond to the results of the SAP. (Section \ref{sec:evaluation_and_response})
\begin{compactitem}
...
We then identify the assessed \textbf{risk of material misstatement}\index{risk of material misstatement} (\textbf{RMM})\index{RMM} to which the SAP is intended to respond. 
...
\bigskip
\noindent Practical examples of plausible relationships are the following.
...
Data \textbf{aggregation}\index{aggregation} and \textbf{disaggregation}\index{disaggregation} refers to how account balances are combined or segmented for testing purposes, such as using daily, weekly or monthly rather than quarterly or annual data. 
...
The first two steps are executed on the \textbf{training set}\index{training set}\index{set!training}, which is the dataset or sample used to construct the model. 
The model is then applied to the \textbf{hold-out set}\index{hold-out set}\index{set!hold-out} to validate the theory, determine the acceptable difference and develop expectations. 
...
The next requirement is the precision evaluation. \textbf{Precision}\index{precision} is a measure of the closeness of expectations to correct amounts. 
...
This range is referred to as the \textbf{prediction interval}\index{prediction interval}\index{interval!prediction}. 
The width of the interval reflects the expectation precision.

Figure \ref{fig:H6_prediction_intervals_precise_less_precise} shows the prediction intervals for a more precise expectation and for a less precise expectation. 
Both distributions are centered on zero.
%  and cover 99\%\footnote{The choice of this percentage is explained on page \pageref{mar:controlling_efficiency_risk}.} of the expected differences. 
%  There is a 0.5\% chance that a difference is observed that is less than $C_{.005}$, and a 0.5\% chance that a difference is observed that is greater than $C_{.995}$.\nomenclature{$C_{\alpha/2}$}{Lower bound of the prediction interval if the population is not misstated, with a confidence level of $1-\alpha$}\nomenclature{$C_{1 - \alpha/2}$}{Upper bound of the prediction interval if the population is not misstated, with a confidence level of $1-\alpha$}

\begin{figure}
  \includegraphics[width=\columnwidth]{H6_prediction_intervals_precise_less_precise.pdf}
  \caption{Comparison of density distributions for a precise model (solid line) and a less precise model (dotted line). Both distributions represent the expected differences.}
  \label{fig:H6_prediction_intervals_precise_less_precise}
\end{figure}
...
\bigskip
Which size difference requires further investigation? The answer to this question hinges on the risk that the auditor wishes to control, and our argument is similar to that discussed in Volume 1, Chapter \ref{cha:hypothesis_testing} regarding hypothesis testing. 

We begin with Step 1 in the nine-step approach\index{nine-step approach} from Volume 1, Section \ref{sec:the_nine_step_approach}: the research question. Section \ref{sub:account_and_assertions} provides details, for example, whether the auditor is concerned about overstatements, understatements, or both.
...
\begin{figure}[htbp]
  \includegraphics[width=\columnwidth]{H6_Expected_diff_precise.pdf}
...
\begin{figure}[htbp]
  \includegraphics[width=\columnwidth]{H6_Expected_diff_less_precise.pdf}
  \caption{Expected difference for a precise model, when the account is understated by $PM$, correct, or overstated by $PM$}
...
For an auditor, \textbf{expectations}\index{expectation} are predictions of recorded accounts or ratios, whereas, for a data scientist, the \textbf{expected value}\index{expected value}\index{value!expected} is the mean of the sampling distribution of a variable $X$, generally denoted as $E(X)$. 
\textbf{Prediction}\index{prediction} is commonly used in the sense of either using data to make a statement about the future, as in forecasting, or (closer to our application of analytical procedures) using data to 'predict' outcomes that have already occurred. 
...
\label{sec:workshop06}

\input{workshop06_R}

% section workshop_06 (end)

\section{Workshop Python} % (fold)
\label{sec:workshop06_python}

\input{workshop06_Python}

% section workshop_06_python (end)
