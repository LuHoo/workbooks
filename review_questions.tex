...
\begin{exercise}
You are given the first-digit frequencies for 900 invoice amounts and a Benford-based expected distribution.
\begin{enumerate}
    \item Compute the contribution of each digit category to the chi-square statistic.
    \item Identify which categories drive most of the test statistic.
    \item Explain how these contributions affect your interpretation of the overall test result.
\end{enumerate}
\end{exercise}

\begin{exercise}
A chi-square goodness-of-fit test shows a statistically significant deviation from the expected distribution.
Draft an audit response memo that:
\begin{enumerate}
    \item distinguishes this statistical result from evidence of fraud;
    \item proposes two plausible non-fraud explanations;
    \item specifies two follow-up audit procedures that would be proportionate to the finding.
\end{enumerate}
\end{exercise}
