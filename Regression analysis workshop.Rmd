...
library(adatheme) # ADA plotting theme
theme_set(theme_ada())
...
hist_revenue <-
  ...
  geom_histogram(binwidth = 4000000, fill = adatheme::colour$ada$get("blue")) +
  scale_x_continuous(labels = label_comma())

hist_production <-
  ggplot(USSteamCoEstim, aes(x = production)) +
  geom_histogram(binwidth = 50000, fill = adatheme::colour$ada$get("blue")) +
  scale_x_continuous(labels = label_comma()) +
  theme(plot.margin = unit(c(0, 1, 0, 0), "cm")) # top, right, bottom, left

hist_coolDD <-
  ggplot(USSteamCoEstim, aes(x = coolDD)) +
  geom_histogram(binwidth = 50, fill = adatheme::colour$ada$get("blue")) +
  scale_x_continuous(labels = label_comma())

hist_heatDD <-
  ggplot(USSteamCoEstim, aes(x = heatDD)) +
  geom_histogram(binwidth = 100, fill = adatheme::colour$ada$get("blue")) +
  scale_x_continuous(labels = label_comma())
...
time_series_plot <- ggplot(data = USSteamCoEstim, aes(x = date)) +
  ...
  theme(
    ...
    axis.title.y.left = element_text(color = adatheme::colour$ada$get("blue")),
    axis.title.y.right = element_text(color = adatheme::colour$ada$get("red"))
  )
time_series_plot
```
...
scatter_plot <- ggplot(USSteamCoEstim, aes(x = production, y = revenue)) +
  geom_point(color = adatheme::colour$ada$get("blue")) +
...
corrplot(cor_ussteam,
  tl.col = "black",
  col = colorRampPalette(c(adatheme::colour$ada$get("red"), "white", adatheme::colour$ada$get("blue")))(100),
  col.lim = c(-1, 1)
)
```
...
summer_colors <- c("No" = adatheme::colour$ada$get("blue"), "Yes" = adatheme::colour$ada$get("red"))
...
make_resid_plot <- function(data, var) {
  ...
  p <- ggplot(data, aes(x = .data[[var]], y = resid)) +
    geom_point(color = adatheme::colour$ada$get("blue"), alpha = 0.7) +
  ...
  if (n_unique > 2) {
    p <- p +
      geom_smooth(
        ...
        formula = y ~ poly(x, 2),
        se = FALSE,
        color = adatheme::colour$ada$get("lightpink")
      )
  } else {
    p <- p +
      geom_smooth(
...
hist_stand_res <-
  ...
  geom_histogram(aes(y = after_stat(density)),
    ...
    fill = adatheme::colour$ada$get("blue")
  ) +
...
regression_lines_plot <- ggplot() +
  geom_line(
    ...
    color = adatheme::colour$ada$get("blue"), alpha = 0.1
  ) + # Plot the simulated regression lines
  geom_line(data = true_line, aes(x = x, y = y), color = adatheme::colour$ada$get("lightpink"), linewidth = .5) + # Plot the true line
  geom_line(
    data = ci_bounds_true, aes(x = x, y = ci_lower),
    color = adatheme::colour$ada$get("lightpink"), linewidth = .5, linetype = "solid"
...
intervals_plot <- ggplot() +
  geom_ribbon(
    ...
    fill = adatheme::colour$ada$get("blue"), alpha = 0.15
  ) + # 99% prediction interval as a ribbon
  geom_line(
    data = ci_bounds_true,
    aes(x = x, y = ci_lower),
    color = adatheme::colour$ada$get("blue"), linewidth = .5, linetype = "dashed"
  ) + # 95% CI lower
  geom_line(
    data = ci_bounds_true,
    aes(x = x, y = ci_upper),
  ...
  geom_line(
    data = true_line,
    aes(x = x, y = y),
    color = adatheme::colour$ada$get("blue"), linewidth = 1
...
expectations_plot <- ggplot(pred_df, aes(x = date)) +
  ...
  geom_ribbon(aes(ymin = lwr, ymax = upr), fill = adatheme::colour$ada$get("blue"), alpha = 0.2) +
  ...
  geom_point(data = subset(pred_df, outside), aes(y = recorded), color = adatheme::colour$ada$get("lightpink"), size = 2) +
  # Define color for predicted and actual values
  scale_color_manual(
    values = c("Expectation" = adatheme::colour$ada$get("blue"), "Recorded" = adatheme::colour$ada$get("lightpink")),
    name = ""
  ) +
  labs(
    x = "Month in 2014",
