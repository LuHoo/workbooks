...
library(scales)          # For editing ggplot graphs
```
...
By default, the residual standard error is displayed with a limited number of significant digits. To obtain a more precise value, one may either temporarily increase the global display precision, bearing in mind that this change will apply to all subsequent output, or extract the residual standard error explicitly as a separate object.

```{r}
summary.mod.1 <- summary(ussteamco.mod.1)
summary.mod.1$sigma
```
...
```{r child='regression-analysis/exercises/cochrane-orcutt.Rmd'}
```
