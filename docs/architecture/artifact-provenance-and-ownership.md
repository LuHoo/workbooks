...
| R distribution workshop notebook (`notebooks/workshops/*.Rmd`) | generated-file header comment includes source path | header indicates generated file | no explicit schema version | deterministic mapping via `scripts/workshop-export-config.R` |
...
- ownership-critical mapping now lives in canonical config script `scripts/workshop-export-config.R`, with `scripts/notebook-manifest.R` retained only as a derived compatibility layer;
