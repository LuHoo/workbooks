...
2. Canonical workshop export config: `scripts/workshop-export-config.R`.
3. Canonical notebook/workshop distribution manifest: `scripts/notebook-manifest.R`.
4. Canonical traceability metadata: `metadata/traceability/*.yml`.
5. Canonical book manuscripts and chapter `.tex` sources at repository root.
...
| Workshop export config | `scripts/workshop-export-config.R` | canonical | human authoring | yes | yes | no |
| Distribution notebook manifest | `scripts/notebook-manifest.R` | canonical | human authoring | yes | yes | no |
...
| R distribution workshop notebook (`notebooks/workshops/*.Rmd`) | generated-file header comment includes source path | header indicates generated file | no explicit schema version | deterministic mapping via `scripts/notebook-manifest.R` |
...
- ownership-critical mapping already exists in canonical config/manifest scripts (`scripts/workshop-export-config.R`, `scripts/notebook-manifest.R`) and architecture docs;
