...
| Binder-facing configuration | `.binder/**` and workbooks binder boundary | human-maintained config + binder readiness tooling | local config edits only; optional smoke command script | `.binder/**`; `notebooks/workshops/.binder/**` | local static review + deferred hosted binder checks | yes | no | yes |
